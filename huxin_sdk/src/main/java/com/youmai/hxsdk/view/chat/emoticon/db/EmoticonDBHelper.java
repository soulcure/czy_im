...
public class EmoticonDBHelper {
    ...
    public synchronized ArrayList<EmoticonSetBean> queryAllEmoticonSet() {
        ...
        return queryEmoticonSet(sql, -1);
    }

    public synchronized ArrayList<EmoticonSetBean> queryAllEmoticonSelf() {
        String sql = "select * from " + TABLE_NAME_EMOTICONSET + " where " + TableColumns.EmoticonSetColumns.NAME + " = 'emoji' OR "
                + TableColumns.EmoticonSetColumns.NAME + " = 'self' ";
        return queryEmoticonSet(sql, 1);
    }

    public synchronized ArrayList<EmoticonSetBean> queryAllEmoticonEmoIj() {
        String sql = "select * from " + TABLE_NAME_EMOTICONSET + " where " + TableColumns.EmoticonSetColumns.NAME + " = 'emoji' ";
        return queryEmoticonSet(sql, 1);
    }

    public synchronized ArrayList<EmoticonSetBean> queryEmoticonSet(String sql, int rowType) {
        SQLiteDatabase db = mOpenDbHelper.getReadableDatabase();
        Cursor cursor = null;
        try {
            ...
            if (count > 0) {
                ...
                for (int i = 0; i < count; i++) {
                    ...
                    EmoticonSetBean bean;
                    if (rowType == 1) {
                        bean = new EmoticonSetBean(1, name, 3, 3, iconUri, isshowdelbtn, isShownName, itempadding, horizontalspacing, verticalSpacing, emoticonList);
                    } else {
                        bean = new EmoticonSetBean(-1, name, line, row, iconUri, isshowdelbtn, isShownName, itempadding, horizontalspacing, verticalSpacing, emoticonList);
                    }
    ...
    private static class DBOpenHelper extends SQLiteOpenHelper {
        ...
        public void onUpgrade(SQLiteDatabase sqLiteDatabase, int oldVersion, int currentVersion) {
            ...
            EmotionInit.resetEmoticonInitStatus(context);
