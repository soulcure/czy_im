package com.youmai.hxsdk.view.camera;
...
import com.youmai.hxsdk.view.camera.listener.TypeListener;
import com.youmai.hxsdk.view.camera.state.CameraMachine;
import com.youmai.hxsdk.view.camera.util.FileUtil;
import com.youmai.hxsdk.view.camera.util.LogUtil;
import com.youmai.hxsdk.view.camera.util.ScreenUtils;
import com.youmai.hxsdk.view.camera.view.CameraView;
...
public class JCameraView extends FrameLayout implements CameraInterface.CameraOpenOverCallback, SurfaceHolder
        .Callback, CameraView, SensorEventListener {
//    private static final String TAG = "JCameraView";
    ...
    protected void onAttachedToWindow() {
        ...
        sm = (SensorManager) getContext().getApplicationContext().getSystemService(Context.SENSOR_SERVICE);
        if (sm != null) {
    ...
    private void initView() {
        ...
        View view = LayoutInflater.from(mContext).inflate(R.layout.hx_camera_view, this);
        ...
        mLeftClose = (ImageView) view.findViewById(R.id.image_close);
        setFlashRes();
        ...
        mCaptureLayout.setCaptureLisenter(new CaptureListener() {
            ...
            public void takePictures() {
                ...
                mLeftClose.setVisibility(INVISIBLE);
            ...
            public void recordShort(final long time) {
                ...
                mLeftClose.setVisibility(VISIBLE);
            ...
            public void recordEnd(long time) {
                //录制结束
                millisecond = time;
                millisecond = time - 100 >= 0 ? time - 100 : 0;//录制的实际效果大约多算100ms,比如，算计10000ms的视频，在播放器会显示9s
...
//        });
        //关闭退出
        mLeftClose.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
    ...
    public void onResume() {
        ...
        postDelayed(new Runnable() {
            @Override
            public void run() {
                setFocusViewWidthAnimation(ScreenUtils.getScreenWidth(mContext) / 2, ScreenUtils.getScreenHeight(mContext) / 2);
            }
        }, 500);
...
//                    Log.i("CJT", "result = " + (result - firstTouchLength));
    ...
    public void confirmState(int type) {
        switch (type) {
            case TYPE_VIDEO:
                ...
                machine.start(mVideoView.getHolder(), screenProp);
                if (jCameraLisenter != null) {
                    jCameraLisenter.recordSuccess(videoUrl, firstFrame, millisecond);
                }
                break;
            case TYPE_PICTURE:
                mPhoto.setVisibility(INVISIBLE);
    ...
    public void showPicture(Bitmap bitmap, boolean isVertical) {
        ...
        //mCaptureLayout.startAlphaAnimation(); //拍照完瞬间显示提示语
    ...
    float focusX, focusY, focusZ;//对焦点
    boolean initFirstSensor;

    boolean isStartFocus = false;
    ...
    public void onSensorChanged(SensorEvent event) {
        ...
        float offset = (float) (Math.pow(Math.abs(focusX - x), 2) + Math.pow(Math.abs(focusY - y), 2) + Math.pow(Math.abs(focusZ - z), 2));
        if (offset > 16) {//计算坐标偏移值
            if (!isStartFocus) {
                float viewX = getMeasuredWidth() / 2;
                float viewY = getMeasuredHeight() / 2;
                setFocusViewWidthAnimation(viewX, viewY);
                focusX = x;
                focusY = y;
                focusZ = z;
            }
