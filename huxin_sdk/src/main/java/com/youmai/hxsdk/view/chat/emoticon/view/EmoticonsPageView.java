...
import com.youmai.hxsdk.view.chat.view.SoftListenLayout;
    ...
    private void updateView() {
        ...
        for (EmoticonSetBean bean : mEmoticonSetBeanList) {
            /*if (bean.getName().equals("self")) {
                ...
                if (bean.getIndex() != 1) {
                    EmoticonBean addBean = new EmoticonBean();
                    addBean.setEventType(EmoticonBean.FACE_TYPE_SELF_ADD);
                    addBean.setIconUri("drawable://hx_emo_add1");
                    addBean.setMsgUri("");
                    addBean.setName("");
...
//                    rl.setBackgroundColor();
                    ...
                    if (bean.isShowDelBtn()) {
                        ...
                        list.add(new EmoticonBean(EmoticonBean.FACE_TYPE_DEL, "drawable://hx_emotion_cancel", null, null));//表情删除图标
                    } else {
                        int count = bean.getLine() * bean.getRow();
    ...
    public void setEmoticonContents(EmoticonsKeyboardBuilder builder, int p) {
        mEmoticonSetBeanList = builder.builder.getEmoticonSetBeanList();
        initPostion = p;
    }
